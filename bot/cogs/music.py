...
class Queue:
    ...
    def set_repeat(self, mode):
        if mode is None:
            ...
            self.repeat = RepeatMode.CURRENT
        elif mode == "all":
...
class Player(wavelink.Player):
    ...
    async def start_playback(self, ctx):
        await ctx.send(f"Now playing **{self.queue.current_track.title}**.")
...
class Music(commands.Cog, wavelink.WavelinkMixin):
    ...
    async def connect_command(self, ctx, *, channel: typing.Optional[discord.VoiceChannel]):
    ...
    async def pause_command_error(self, ctx, exc):
        if isinstance(exc, PlayerIsAlreadyPaused):
            ...
            await ctx.send("Player is currently not playing anything.")
    ...
    async def next_command(self, ctx):
        ...
        if not player.queue.upcoming and player.queue.repeat == RepeatMode.NONE:
            raise NoMoreTracks
    ...
    async def shuffle_command_error(self, ctx, exc):
        if isinstance(exc, QueueIsEmpty):
            await ctx.send("The queue could not be shuffled as the queue is currently empty.")


    @commands.command(name="repeat")
    async def repeat_command(self, ctx, mode: typing.Optional[str]):
        if not (mode is None or mode in ("none", "current", "all")):
            raise InvalidRepeatMode

        player = self.get_player(ctx)
        player.queue.set_repeat(mode)

        await ctx.send(f"Repeat is set to **{str(player.queue.get_repeat())[11:]}**.")
    ...
    async def queue_command(self, ctx, show: typing.Optional[int] = 10):
        ...
        embed.add_field(
            ...
            value=f'[{getattr(player.queue.current_track, "title", "No tracks currently playing.")}]({getattr(player.queue.current_track, "uri", None)})',
            inline=False
        )
        if upcoming := player.queue.upcoming:
            embed.add_field(
                name="Next up",
                value="\n".join(f"`{i+1}.`\t[{t.title}]({t.uri})" for i, t in enumerate(upcoming[:show])),
                inline=False
            )
